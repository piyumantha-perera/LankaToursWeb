(function ($) {
  ...
  /*---------- 01. On Load Function ----------*/
    ...
    // Function to add animation classes
     ...
     $('.th-slider').each(function () {
        var thSlider = $(this);
        var dataOptions = thSlider.attr('data-slider-options');
        var settings = {};
        ...
        // Initialize thumbs if defined
        if (settings.thumbs && typeof settings.thumbs.swiper === 'string') {
            try {
                settings.thumbs.swiper = new Swiper(settings.thumbs.swiper, {
                    slidesPerView: 3,
                    spaceBetween: 10,
        ...
        // Merge final options and initialize Swiper
        var options = $.extend({}, sliderDefault, settings);
        if (thSlider.length && thSlider.get(0) !== null) {
            try {
        ...
        // Add arrow-wrap class if slider-area exists
        if ($('.slider-area').length > 0) {
            $('.slider-area').closest(".container").parent().addClass("arrow-wrap");
        }
    
        // Category-specific wheel effect
        function categoryWheelEffect(slideSelector, multiplier) {
            function calculateWheel() {
        ...
        if (thSlider.hasClass('categorySlider')) {
            categoryWheelEffect('.single', { translate: 0.1, rotate: 0.01 });
        }
    
        if (thSlider.hasClass('categorySlider6')) {
            categoryWheelEffect('.single3', { translate: 0.1, rotate: 0 });
        }
    });
